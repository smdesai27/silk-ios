...
struct FuzzCorpusR1 {
    ...
    func row(_ row: FuzzCorpusRow) {
        ...
        do {
            ...
            // Not mechanizable — skipped, never guessed. Grep FUZZR1-SKIP.
            print("FUZZR1-SKIP \(row.source)#\(row.index): \(error.reason)")
            return
        } catch {
            print("FUZZR1-SKIP \(row.source)#\(row.index): \(error)")
            return
        }
        if let mismatch {
            Issue.record("FUZZR1-FAIL \(row.source)#\(row.index) | utterance=\(sanitize(row.utterance)) | \(mismatch)")
        }
    }
}

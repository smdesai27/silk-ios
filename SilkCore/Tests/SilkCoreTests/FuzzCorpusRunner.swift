...
// THE FUZZ-CORPUS INTERPRETER, ONCE.
//
// The generated corpora are written in a small expectation language — a head
// like `PARSE SPEND door=tiktok minutes=20` or `VERDICT GRANT minutes=10`, plus
// a state and clock precondition — and this file is the thing that reads it.
// Every mechanizable row of every corpus is driven through the real pipeline,
// `DeterministicParser.parse` → `Validator.validate`, and compared against what
// its spec says. A row whose spec cannot be interpreted is SKIPPED LOUDLY and
// never guessed at.
//
// It lived twice. `FuzzCorpusR1.swift` and `FuzzCorpusR2.swift` carried 476
// byte-identical lines of it, and R2's own header said why: "the R1 helpers are
// all file-private there and the R1 files are frozen, so the machinery is
// duplicated verbatim rather than modified." That is an honest account of how
// it happened and it is still two copies of a spec reader — which is the one
// kind of duplication a fuzz suite cannot afford, because the interpreter is
// what decides whether a corpus row PASSED. A fix applied to one copy and not
// the other does not make a test fail; it makes two corpora disagree about what
// their own language means, silently, in the direction of green.
//
// The frozen thing was never the interpreter. It is the R1 DATA — the rows and
// their expectations — and that is untouched in `FuzzCorpusR1Data.swift`.
// Moving the reader here changes no row's verdict; the round files keep their
// `@Suite`s and their grep tags and nothing else.
...
private func build(_ row: FuzzCorpusRow) throws -> (head: String, built: BuiltCase) {
    ...
    for (d, n, t) in live {
        let issued = julyAt(t.hour, t.minute)
        ledger.record(Grant(door: d, minutes: n, issuedAt: issued,
                            expiresAt: issued.addingTimeInterval(Double(n) * 60)))
    }
    for (d, t) in closed {
        ledger.closeDoor(d, at: now.addingTimeInterval(-600), until: t.map { julyAt($0.hour, $0.minute) })
    }
...
private func verdictMismatch(_ row: FuzzCorpusRow, head: String, built: BuiltCase) throws -> String? {
    ...
    func resolveUntil(_ s: String) throws -> Date {
        ...
        var candidate = julyAt(t.hour, t.minute)
    ...
    case "GRANT":
        ...
        if let r = kv["relock"] {
            ...
            if r.hasPrefix("+"), r.hasSuffix("m") {
                ...
                expectedRelock = julyAt(t.hour, t.minute)
            }
...
/// Drives one corpus row. `tag` prefixes the skip and fail markers
/// (FUZZR1-SKIP, FUZZR2-FAIL, …) so a grep stays per-suite — which is the only
/// thing that ever differed between the two copies of this function.
func runRow(_ row: FuzzCorpusRow, tag: String) {
    let mismatch: String?
    do {
